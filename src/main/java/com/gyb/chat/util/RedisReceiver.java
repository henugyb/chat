...
public class RedisReceiver {
    ...
    public void onOpen(Session session, long userid){
        ...
        synchronized (list) {
            ...
            System.out.println(user.getUsername()+"上线了："+list.size());
        ...
        synchronized (list){
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if(info.session.getId().equals(session.getId())){
                    ...
                    User user = userDao.findById(info.userid).get();
                    System.out.println(user.getUsername()+"下线了："+list.size());
